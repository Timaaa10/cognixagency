...
const Hero: React.FC<HeroProps> = ({ onBookClick, onAuditClick }) => {
  const industries = [
    'LAW FIRMS', 
    'POOL BUILDERS', 
    'HVAC & SOLAR', 
    ...
    <section className="relative pt-24 pb-24 lg:pt-32 lg:pb-40 overflow-hidden transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        ...
        <div className="max-w-3xl mx-auto mb-10 animate-fade-in-up">
          <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/60 dark:bg-navy-800/50 backdrop-blur-md border border-slate-200 dark:border-navy-700 shadow-sm">
            ...
            <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">
              Stop just selling ads. <span className="text-[#0a1d37] dark:text-white font-black">Scale your revenue.</span>
            </span>
          </div>
        </div>
        ...
        <p className="text-xl md:text-2xl font-medium text-slate-700 dark:text-slate-300 mb-12 max-w-3xl mx-auto leading-relaxed">
          The scaling engine for service-based businesses. Combining high-converting infrastructure, AI-powered sales systems, and premium demand generation to pack your calendar with qualified clients.
        </p>
        ...
        <div className="mt-24 pt-12 border-t border-slate-200 dark:border-navy-800/50 max-w-6xl mx-auto">
          <div className="mb-10">
            <p className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em] mb-4">THE REVENUE STANDARD FOR</p>
            <div className="inline-flex flex-wrap items-center justify-center gap-3 px-6 py-4 rounded-[2rem] bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/50 shadow-sm mb-6">
              <span className="text-blue-600 dark:text-blue-400 text-base md:text-lg font-black tracking-wide">
                TRT (Testosterone Replacement Therapy) Clinics
              </span>
              <span className="hidden md:inline text-slate-300 dark:text-navy-800">|</span>
              <span className="text-slate-600 dark:text-slate-400 text-sm font-bold">
                Our Core Niche & Performance Leader
              </span>
            </div>
          </div>
          <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-6">Also serving high-ticket local services</p>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6 md:gap-8 opacity-70 dark:opacity-50 hover:opacity-90 transition-all duration-300">
            {industries.filter(item => item !== 'MEDICAL & DENTAL' && item !== 'AESTHETIC CLINICS').map((item, i) => (
              <span key={i} className="text-[11px] md:text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors cursor-default whitespace-nowrap tracking-wider">
                {item}
              </span>
            ))}
          </div>
